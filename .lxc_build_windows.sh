...
lxc exec ${CONTAINER} -- bash -c 'apt-get install --assume-yes imagemagick icoutils'
...
lxc exec ${CONTAINER} -- bash -c 'tar --create --file /tmp/work/build/build_windows_x86.tar.gz --gzip --directory /tmp/work/build/windows32/lua5.1/lua/install .'
lxc file pull ${CONTAINER}/tmp/work/build/build_windows_x86.tar.gz build/

# Build the 64bit version.
lxc exec ${CONTAINER} -- bash -c 'export PATH=/usr/mingw-w64-x86_64/bin:${PATH} && cd /tmp/work && bash .build02_windows64.sh'
lxc exec ${CONTAINER} -- bash -c 'tar --create --file /tmp/work/build/build_windows_x86_64.tar.gz --gzip --directory /tmp/work/build/windows64/lua5.1/lua/install .'
lxc file pull ${CONTAINER}/tmp/work/build/build_windows_x86_64.tar.gz build/
