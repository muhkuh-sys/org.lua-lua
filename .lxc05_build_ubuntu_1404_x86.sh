...
lxc launch mbs-ubuntu-1404-x86 ${CONTAINER} -c security.privileged=true
...
lxc exec ${CONTAINER} -- bash -c 'tar --create --file /tmp/work/build/build_ubuntu_1404_x86.tar.gz --gzip --directory /tmp/work/build/linux/lua5.1/lua/install .'
lxc exec ${CONTAINER} -- bash -c 'chown `stat -c %u:%g /tmp/work` /tmp/work/build/build_ubuntu_1404_x86.tar.gz'
