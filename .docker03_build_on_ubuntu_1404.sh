...
docker exec ${ID} bash -c 'tar --create --file /tmp/work/build/build_ubuntu_1404_x86.tar.gz --gzip --directory /tmp/work/build_linux32/lua5.1/lua/install .'
docker exec ${ID} bash -c 'chown `stat -c %u:%g /tmp/work` /tmp/work/build/build_ubuntu_1404_x86.tar.gz'

# Build the 64bit version.
docker exec ${ID} bash -c 'cd /tmp/work && bash .build04_linux64.sh'
docker exec ${ID} bash -c 'tar --create --file /tmp/work/build/build_ubuntu_1404_x86_64.tar.gz --gzip --directory /tmp/work/build_linux64/lua5.1/lua/install .'
docker exec ${ID} bash -c 'chown `stat -c %u:%g /tmp/work` /tmp/work/build/build_ubuntu_1404_x86_64.tar.gz'
