...
lxc init mbs-ubuntu-1704-x86 ${CONTAINER} -c security.privileged=true
...
lxc exec ${CONTAINER} -- bash -c 'tar --create --file /tmp/work/build/build_ubuntu_1704_x86.tar.gz --gzip --directory /tmp/work/build/linux/lua5.2/lua/install .'
lxc exec ${CONTAINER} -- bash -c 'chown `stat -c %u:%g /tmp/work` /tmp/work/build/build_ubuntu_1704_x86.tar.gz'
